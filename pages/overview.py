"""
Overview Page — KPI cards + rating distribution + top cuisines.
...
# Cuisine categories used to filter non-cuisine tags
CUISINE_CATEGORIES = {
    "Italian", "Chinese", "Mexican", "Japanese", "Thai", "Indian", "French",
    "Greek", "Mediterranean", "Vietnamese", "Korean", "American (Traditional)",
    "American (New)", "Middle Eastern", "Lebanese", "African", "Caribbean",
    "Latin American", "Spanish", "Pakistani", "Sri Lankan", "Filipino",
    "Himalayan/Nepalese", "Persian/Iranian", "Turkish", "Ethiopian", "Taiwanese",
    "Cantonese", "Szechuan", "Hong Kong Style Cafe", "Seafood", "Steakhouses",
    "Sushi Bars", "Burgers", "Sandwiches", "Pizza", "Bakeries",
    "Breakfast & Brunch", "Diners", "Delis", "Tapas Bars", "Tapas/Small Plates",
    "Noodles", "Ramen", "Hot Pot", "Dim Sum", "Chicken Wings", "Barbeque",
    "Soup", "Waffles", "Poutineries", "Creperies", "Fondue", "Soul Food",
    "Comfort Food",
}
...
def compute_cuisine_stats(city: str, segment: str) -> pd.DataFrame:
    ...
    if df_exp.empty:
        return pd.DataFrame(columns=["cuisine", "avg_stars", "total_reviews", "business_count"])
    return (
...
layout = html.Div([
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.Div([
                html.Div([
                    html.Span(id="cuisine-chart-title", className="section-title",
                    ...
                    dcc.RadioItems(
                        ...
                        labelStyle={"marginRight": "14px", "fontSize": "0.82rem", "cursor": "pointer"},
                        style={"display": "inline-block", "marginLeft": "auto"},
                    ),
                ], style={"display": "flex", "alignItems": "center",
                          "justifyContent": "space-between", "marginBottom": "0.75rem",
                          "borderBottom": "1px solid #eaf0f9", "paddingBottom": "0.5rem"}),
                dcc.Graph(id="top-cuisines-chart", config={"displayModeBar": False}),
            ], className="chart-card"),
        ], md=7),
    ], className="mb-3"),
    ...
    dbc.Row([
        dbc.Col([
            html.Div([
                ...
                dcc.Graph(id="cuisine-bubble-chart", config={"displayModeBar": False}),
            ], className="chart-card"),
        ]),
    ]),
...
def update_kpis(city, segment):
    ...
    cards = [
        {"label": "Total Businesses",   "value": f"{total_biz:,}",
         "sub": city if city != "Both" else "Toronto + Montreal", "border": "#2c7be5"},
        {"label": "Avg Star Rating",    "value": f"{avg_stars:.2f} ★",
         "sub": "across filtered businesses", "border": "#f6c23e"},
        {"label": "Most Popular Cuisine", "value": top_cuisine,
         "sub": "by total reviews", "border": "#1cc88a"},
        {"label": "Best Rated Cuisine", "value": rated_cuisine,
         "sub": rated_stars, "border": "#D52B1E"},
    ]
...
def update_rating_dist(city, segment):
    ...
    fig.update_layout(
        ...
        legend=dict(title=""), xaxis=dict(dtick=0.5), bargap=0.05, height=300,
    )
    return fig
...
def update_top_cuisines(city, segment, metric):
    ...
    fig = px.bar(
        top10, x=metric, y="cuisine", orientation="h",
        color=metric, color_continuous_scale=["#d0e8ff", color],
        text=metric, labels={metric: label, "cuisine": ""},
    ...
    fig.update_layout(
        ...
        yaxis=dict(tickfont=dict(size=11)), height=300,
    )
    return fig, title
...
def update_cuisine_bubble(city, segment, metric):
    ...
    fig = px.scatter(
        top,
        x="business_count", y="avg_stars",
        size="total_reviews", color="avg_stars",
        color_continuous_scale="RdYlGn", range_color=(3.0, 4.5),
        text="cuisine", size_max=55,
        labels={
            "business_count": "Number of Businesses",
