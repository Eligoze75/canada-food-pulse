...
canada-food-pulse/
│
├── app.py                        # Dash app entry point; navbar, layout, server export
│
├── pages/
│   ├── overview.py               # Overview page (KPIs, cuisines, bubble chart)
│   ├── peak_hours.py             # Peak Hours page (heatmap, busiest day)
│   └── map_view.py               # Map page (density map, neighbourhood KPIs)
│
├── scripts/
│   └── preprocess.py             # One-time preprocessing: generates processed data files
│
├── notebooks/
│   ├── eda.ipynb                 # Exploratory data analysis notebook
│   └── text_utils.py             # City name cleaning utilities used in EDA
│
├── data/
│   └── processed/                # Generated by scripts/preprocess.py
│       ├── yelp_business_data_cleaned.csv   # Businesses (Toronto + Montreal) × weekday peaks
│       ├── df_businesses.csv                # One row per business with aggregate checkin stats
│       ├── df_cuisine_stats.csv             # Cuisine × city aggregates
│       └── df_peak_heatmap.csv              # City-level weekday × hour check-in totals
│
├── assets/
│   ├── style.css                 # Global CSS (layout, KPI cards, navbar, chart cards)
│   └── canada_leaf.png           # Maple leaf logo served by Dash
│
├── img/
│   └── canada_leaf.png           # Source image asset
│
├── environment.yml               # Conda environment (local development)
├── requirements.txt              # Pip packages (Plotly Cloud / gunicorn deployment)
├── Procfile                      # Tells Plotly Cloud to run: gunicorn app:server
└── README.md
```
